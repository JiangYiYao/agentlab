...
- 若用 LLM 裁判：哪条命令、以及 **几次独立审查**（LLM 分条数 × 这次新跑的试验数，写成数字）。用户只点模型名不够。现在做不到一个裁判并排看几份 diff，不要那样写。
...
只能看改得像不像、注释好不好时，用 `llm_rubric`，**不能**当必须满足的条目。裁判工作目录就是代码快照根（不含 `.git`），stdin 带该用例 `prompt.md`、标准前言 + 当前节、`trial_id` / `case_id`、改动文件列表。评什么以任务说明和标准为准。
