...
表后面再补：看哪些关注点、怎样算过、产物放哪。默认不设墙钟、金额或 token 上限。跑测试的人自己清楚消耗；硬上限只会把还在跑的试验掐掉。用户没要求就不要写 `wall_clock_s` / `usd` / `tokens`，也不要自己估一个。只有他主动要求时才写进契约。
...
进入实验计划之后，只补还缺的事实：测哪个目录、本机哪条已经能登录使用的命令、任务说明怎么喂给这条命令（默认从标准输入读；用户没确认不要猜 `claude`/`codex`）、新闻是用已经存好的还是上网现搜。对用户不要说「夹具」。这些问句跟在实验计划后面；改法还没谈完时不要问。不要把预算当成必问题，默认不设上限。
...
`brief` 退出 2：读错误码，你改契约或再问缺的那一项，不要把栈甩给用户。`run --gate` 退出 1：说明哪一点不满足、所以这版不能用。退出 3：预算到了，实验没跑完。
